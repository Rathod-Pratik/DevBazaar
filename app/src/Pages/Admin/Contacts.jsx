  ...
  const filterSearch = (searchValue) => {
    ...
    if (lowerValue === "") {
      ...
      const filtered = contact.filter((contactdata) =>
        contactdata.userInfo.name.toLowerCase().includes(lowerValue)
      );
  ...
  {typeof FilterContactData === "undefined" ? (
    ...
    FilterContactData.map((Contect, index) => (
      <tr className="bg-white border-b border-gray-200" key={index}>
        ...
        <th className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{Contect.userInfo.FirstName} {Contect.userInfo.LastName}</th>
        <td className="px-6 py-4">{Contect.name}</td>
        <td className="px-6 py-4">{Contect.number}</td>
